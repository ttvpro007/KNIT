...
const ThreeDButton = ({ position = [0, 0, -2], color = 0xff0000, size = [0.5, 0.2, 0.1], action = 'start' }) => {
    const mountRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const onClick = (event) => {
            ...
            const intersects = raycaster.intersectObject(button);
            if (intersects.length > 0) {
                console.log(`3D Button Clicked! Action: ${action}`);

                // Call Delight VR API based on action
                const videoElement = document.querySelector("dl8-video");
                if (videoElement) {
                    if (action === "start") {
                        videoElement.start();
                    } else if (action === "exit") {
        ...
        const animate = () => {
            requestAnimationFrame(animate);
            // button.rotation.y += 0.01; // Add rotation effect
            renderer.render(scene, camera);
        };
        animate();
